...
public class PreviousController extends PhictController {
    private static ActivityManagerService activityManager =
        ...
        public Navigation run() throws Exception {
            if (requestParameterExists("activity") && requestParameterExists("key")) {
                ...
                return showPreviousResult(resultKey, activityKey);
            }
    ...
    private Navigation showPreviousResult(Key resultKey, Key activityKey) {
        ...
        for (int i = 0; i < results.size(); i++) {
            if (results.get(i).getId().equals(result.getId())) {
                if (i == 0) {
                    requestScope("result", results.get(results.size() -1));
                } else {
